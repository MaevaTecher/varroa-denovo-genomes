...
**Background:** Varroa mites are specialist ectoparasites of _Apis_ honey bees, native from Asia. Within the cryptic genus, two sister species <span style="color:red"> _Varroa destructor_ </span> and <span style="color:blue">  _Varroa jacobsoni_ </span> have drawn attention as they both independently and repeatdely switched host from the Eastern honey bee _Apis cerana_ to the Western honey bee _A. mellifera_. 
...
``` {r layer, warning=FALSE, results="hide", cache = TRUE}
...
The map is interactive: it is possible to zoom in and out, and passing the cursor on one point will give basic information on the year and reporter. On the top right corner of the map, a control button allow to add/remove layers containing mtDNA haplogroup information for each Varroa species.  
...
``` {r layersmap, warning=FALSE, cache = TRUE}
#Data with exact or approximate GPS coordinates obtained from references
coordvarroa <- read.csv("data/mappingpoints_Varroacerana2019.csv", header = TRUE)
...
``` {r orthogroup, cache = TRUE}
tableS1 <- read.csv("data/tableS01-orthogroups.csv")
...
``` {r details orthogroup, cache = TRUE}
kable(cbind(tableS1)) %>%
...
``` {r tree, cache = TRUE}
acariphylo <- read.tree("data/tree_rooted.nwk")
...
``` {r orthogroup acari, cache = TRUE}

tableS2 <- read.csv("data/tableS02-goshared.csv")
...
``` {r karyogram, message=FALSE, warning=FALSE, results="hide", cache = TRUE}
...
regionsvd <- read.csv("data/Vd-location-17072018-short.csv")

regionsvj <- read.csv("data/Vj-location-17072018-short.csv")

common <- read.csv("data/Varroa-ancestor-20072018-short.csv")
...
#pdf(file = "data/VDVJ-POSSEL.pdf", width = 20, height = 12)
...
```{r label, out.width = "100%", cache = TRUE}
knitr::include_graphics("data/VDVJ-POSSEL.png")
```


## Shared genes between the sister Varroa species

The following table list all 40 genes under positive selection shared only by both Varroa mite genomes. Possible gene implicated in tolerance to external stressors or stimuli (Table 2) is indicated in red.

``` {r varroa possel, cache = TRUE}
tableS3 <- read.csv("data/tableS03-posselvarroa.csv")
...
``` {r vdes possel, cache = TRUE}
tableS4 <- read.csv("data/tableS04-posvd.csv")
...
``` {r vjac possel, cache = TRUE}
tableS5 <- read.csv("data/tableS05-posvj.csv")
...
#Preparing the data
Vd.data <-read.csv("data/vdesgoassoc.csv")
#dim(Vd.data)
#summary(Vd.data)
#Vd.data$Gene.id
...
#write.csv(Vd.melt2, file = "data/VdesGOready.csv")
...
annot.vd <- read.csv("data/VdesGOready2.csv") 
...
genesposel.vd <- read.csv("data/Vdesselected1511.csv")
...
#GO_enrich.vd %>% 
#  arrange(Pvalue) %>% 
#  write.csv(file = "data/GO_term_enrichment_VdBP-15112018.csv")

#### V. jacobsoni

Vj.data <-read.csv("data/vjacgoassoc.csv")
#dim(Vj.data)
#summary(Vj.data)
#Vj.data$Gene.id
...
#write.csv(Vj.melt2, file = "data/VjacGOready.csv")

## In the csv I removed the LOC before sequence number to avoid problems due to character and integerFALSE compatibility

#Enrichment with GOstats
annot.vj <- read.csv("data/VjacGOready2.csv") 
...
genesposel.vj <- read.csv("data/Vjacselected1511.csv")
...
#GO_enrich.vj %>% 
#  arrange(Pvalue) %>% 
#  write.csv(file = "data/GO_term_enrichment_VjBP-15112018.csv")
...
tableS6 <- read.csv("data/tableS06-posGOterm.csv")
...
tableS7 <- read.csv("data/tableS07-duplicatediff.csv")
...
tableS8 <- read.csv("data/tableS08-duplicateshared.csv")
...
``` {r duplication, cache = TRUE}

duplivdvj <- read.csv("data/Vdvj-duplicate-06102018-short.csv")

duplivd <- read.csv("data/Vd-duplicate-06102018-short.csv")

pdf(file = "data/VDVJ-DUPLICATION.pdf", width = 20, height = 12)
...
tableS9 <- read.csv("data/tableS09-duplicategoVD.csv")
...
tableS10 <- read.csv("data/tableS10-duplicategoVJ.csv")
...
tableS11 <- read.csv("data/tableS11-duplicategoshared.csv")
